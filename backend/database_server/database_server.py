...
class DrinkListApi(Resource):
    def get(self):
        ...
        if 'pourable' in args and args['pourable'] == 1:
            print("Pourable")
            list_of_drinks = []
            valves = Valve.objects(Q(ingredient__exists=True) & Q(
                ingredient__ne=None)).values_list('ingredient')
            print(valves)
            if not valves:
                print("Not valves")
                return []
            ...
            for drink in drinks.all_fields():
                ...
                for di in drink.ingredients:
                    if not di.required:
                        continue
                    if di.ingredient not in valve_names:
                        print(di.ingredient)
                        pourable = False
                        break
                if not pourable:
    ...
    def get(self):
        ...
        if(args['resolve']):
            ...
            for valve in valves:
                ing = None
                if(valve.ingredient):
                    # Convert ingredient reference to dict, and make
                    # id a string
                    ing = valve.ingredient.to_mongo().to_dict()
                    ing["id"] = str(valve.ingredient.id)
                    del ing["_id"]
                valve_output.append(
                    {"id": str(valve.id), "pin": valve.pin, "ingredient": ing})
            return valve_output
        else:
            valves = Valve.objects.all()
